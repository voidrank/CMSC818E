...
func putData(key string, dataBlock []byte) error {
	...
	return dfs.ChunkPut(key, dataBlock)
}

func getData(key string) ([]byte, error) {
	return dfs.ChunkGet(key)
}
...
func getNode(name string) *DNode {
	b, err := dfs.ChunkGet(name)
	if err != nil {
		return nil
	}
...
func (n *DNode) getTimedVersion(tm time.Time) *DNode {
	...
	if n2 := getNode(n.PrevSig); n2 != nil {
...
func main() {
	...
	dfs.Init()
	Init(debug, compress, mount, newfs != "", storePath, tm, readOnly, ldbCache)
}
